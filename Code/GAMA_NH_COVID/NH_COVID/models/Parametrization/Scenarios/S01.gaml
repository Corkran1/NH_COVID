/**
* Name: Parameters (Scenario 01: 50/50)
...
global{
	 ...
	 */
	 ...
	 // ===========| Disease Parameters |=========
	 float GlobalShedding_p <- 0.45; // Baseline 0.5(0.3, 0.7)
	 float GlobalInfection_p <- 0.45; // Baseline 0.5(0.3, 0.7)
	 float Asymptomatic_p <- 0.25; // Basleine 0.25
	 float Introduction_p <- 0.1; // Baseline 0.01(0.05, 0.1)
	 float Hosp_rate <- 0.11;
	 float Res_Hospitalization <- 0.25/(24*5); // 
	 ...
	 // ~~~~~~Vaccination~~~~~
     float Vaccination_OR_S <- 0.0441;
	 float Vaccination_OR_R <- 0.1357;
	 float p_vaccination_res <- 0.75;
	 float p_vaccination_staff <- 0.50;
