...
!!!!! Check reparametrization of PPE use distribution (suggested: 0.9 staff, 0.75 residents) and infection rates (start from 0.5)
...
Plots(DL, main = 'Testing Frequency') %>%
...
Plots(DL, main = 'Vaccine Distribution') %>%
...
CMS_CA <- read.csv("../../Data/CMS/COVID-19_Nursing_Home_Dataset-2.csv") %>%
...
Bbs <- Bb %>% filter(day < 40)

BIp <- PlotCycles(Bbs, var = "I", col = "red3")
BCIp <- Bbs %>%
  mutate(CumI = Cumulative_I_r + Cumulative_I_s) %>% 
  PlotCycles(., var = 'CumI', col = 'red3')

ggarrange(pI, pCI, BIp, BCIp)
```
