...
| $X_R$        | Subset of population representing residents |
| $X_{VF}$     | Visitors at the facility                    |
| $X_{VC}$     | Visitors at the community                   |
| $X_{SF}$     | Staff at the facility                       |
| $X_{SC}$     | Staff at the community                      |
  
  
  
# Model adaptation
  
PNAs revisions incorporated:Models Changed
For all models we have SEAIR but we considere D_R for residents.
R0 values (R0=2-4)

```{r Scenarios}
#function [ts,S_R,E_R,I_R,A_R,D_R,R_R,...
#         S_VC,E_VC,A_VC,I_VC,R_VC,S_VF,E_VF,A_VF,I_VF,R_VF,...
#         S_SC,E_SC,A_SC,I_SC,R_SC,S_SF,E_SF,A_SF,I_SF,R_SF,S_C,E_C,A_C,I_C,R_C]=poissonRes
...
#Beta values
for(k in 1:11){
  values <- seq(from = 0.61, to = 1.21, by = 0.6)
  beta <- values(k);
}
...
parfor realizations=1:100
    ...
    for i=1:1:length(ts)-1
        ...
        event1  =poissrnd(dt*L_in*S_R(i))/dt;           # S -> E
        event2  =poissrnd(dt*m*phi*E_R(i))/dt;          # E -> I
        event3  =poissrnd(dt*(1-m)*phi*E_R(i))/dt;      #event 3 S -> R?
        event4  =poissrnd(dt*gamma_A*A_R(i))/dt;        # A -> R
        event5  =poissrnd(dt*gamma_I*I_R(i))/dt;        # I -> R
        event6  =poissrnd(dt*theta_h*I_R(i))/dt;        # I -> H
        event52 =poissrnd(dt*theta_u*H_R(i))/dt;        # H -> C
        event53 =poissrnd(dt*gamma_h*H_R(i))/dt;        # H -> R
        event54 =poissrnd(dt*gamma_u*U_R(i))/dt;        # C -> R
        event55 =poissrnd(dt*mu_h*H_R(i))/dt;           # H -> D
        event56 =poissrnd(dt*mu_u*U_R(i))/dt;           # C -> D
